...
app.post("/course/publish", (req, res) => {
	const { title, description, tags, author, thumbnail_path } = req.body;
	let errors = [];
	...
	if (errors.length > 0) {
		...
		let response = fetch("https://cryptic-activist-backend.herokuapp.com/courses/course/id");
		const id = uuidv4();
		const type = "course";
...
app.post("/course-module/create", (req, res) => {
	const { course_id, lessons, title, description, author } = req.body;
	let errors = [];
